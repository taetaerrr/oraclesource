-- 주석 -- 사용자 생성 시 특정 문자열로 시작하는 user 생성을 안하겠음
-- hr(c##hr)
...
-- CREATE TABLE, CREATE VIEW, CREATE SESSION => 원래는 CONNECT 롤에 들어 있었음

-- 1) CONNECT 롤 : CREATE SESSION 만 가지고 있음
-- 2) RESOURCE 롤 : CREATE TABLE, CREATE SEQUENCE, CREATE TRIGGER, CREATE PROCEDURE 권한이 들어있음
-- 사용자 생성 후 CONNECT와 RESOUCE 두개를 부여함
